...
class RunCoordinator:
    ...
    def mark_ready_nodes(self, session: ResearchSession, run_id: str) -> bool:
        ...
        for node in self.runtime.orchestrator.next_wave(session.task_graph):
            ...
            changed = True
            self.runtime.dispatch_queue.enqueue_ready_task(run_id, node.node_id)
            self.runtime.database.append_event(
                "task.ready",
...
class LeaseManager:
            ...
            if session.task_graph:
                for node in session.task_graph.nodes:
                    if node.status == "ready":
                        self.runtime.dispatch_queue.enqueue_ready_task(run.run_id, node.node_id)
    ...
    def _apply_execution_failure(
        ...
        run.result = {"summary": "Run failed during execution.", "reason": recovery_summary}
    ...
    def next_dispatch_for_worker(self, worker) -> Optional[DispatchEnvelope]:
        for _ in range(max(1, self.runtime.dispatch_queue.ready_queue_depth())):
            candidate = self.runtime.dispatch_queue.pop_ready_task()
            if candidate is None:
                return None
            run_id, task_node_id = candidate
            try:
            ...
            if not self.runtime._worker_matches_node(worker, session, node):
                self.runtime.dispatch_queue.requeue_ready_task(run.run_id, node.node_id)
                continue
            try:
